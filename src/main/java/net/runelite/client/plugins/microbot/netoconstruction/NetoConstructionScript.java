...
import net.runelite.client.plugins.microbot.netoconstruction.NetoConstructionConfig;
...
public class NetoConstructionScript extends Script {

    private static final int DEFAULT_DELAY = 600;
    private NetoConstructionState state = NetoConstructionState.Idle;
    ...
    public boolean run(NetoConstructionConfig config) {
    ...
    public void grabPlanksWhileWeBuild(NetoConstructionConfig config, int actionDelay){
        if(getButler() != null) {
    ...
    private void calculateState(NetoConstructionConfig config) {
    ...
    private void buildSpace(NetoConstructionConfig config, int actionDelay) {
    ...
    private void removeSpace(NetoConstructionConfig config, int actionDelay) {
    ...
    private void butler(NetoConstructionConfig config, int actionDelay) {
        ...
        if (!butlerIsInteracting) {
            ...
            sleepUntil(() -> Rs2Widget.isWidgetVisible(7602207), 5000);
            Rs2Widget.clickWidget(7602207);

            sleepUntil(() -> Rs2Widget.isWidgetVisible(24248342), 5000);
    ...
    private boolean hasRemoveInterfaceOpen(NetoConstructionConfig config) {
        switch (config.selectedMode()) {
            case OAK_DUNGEON_DOOR:
