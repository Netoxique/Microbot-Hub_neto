...
import net.runelite.client.plugins.microbot.netowoodcutting.Forestry.*;
import net.runelite.client.plugins.microbot.netowoodcutting.enums.ForestryEvents;
...
@PluginDescriptor(
        name = "Neto Woodcutting",
        description = "Microbot woodcutting plugin",
        tags = {"Woodcutting", "microbot", "skilling"},
        authors = {"Neto"},
        version = NetoWoodcuttingPlugin.version,
        minClientVersion = "2.1.32",
        cardUrl = "https://chsami.github.io/Microbot-Hub/NetoWoodcuttingPlugin/assets/card.jpg",
        iconUrl = "https://chsami.github.io/Microbot-Hub/NetoWoodcuttingPlugin/assets/icon.jpg",
        enabledByDefault = PluginConstants.DEFAULT_ENABLED,
        isExternal = PluginConstants.IS_EXTERNAL
)
@Slf4j
public class NetoWoodcuttingPlugin extends Plugin {
    public static final String version = "1.9.3";
    @Inject
    @Getter(AccessLevel.MODULE)
    public NetoWoodcuttingScript netoWoodcuttingScript;
    @Inject
    public NetoWoodcuttingConfig config;
    @Inject
    private OverlayManager overlayManager;
    @Inject
    private NetoWoodcuttingOverlay woodcuttingOverlay;
    ...
    @Provides
    NetoWoodcuttingConfig provideConfig(ConfigManager configManager) {
        return configManager.getConfig(NetoWoodcuttingConfig.class);
    }
    ...
    protected void startUp() throws AWTException {
        ...
        if (config.enableForestry())
            this.addEvents();
        netoWoodcuttingScript.run(config);
    }

    protected void shutDown() {
        netoWoodcuttingScript.shutdown();
        this.removeEvents();
        ritualCircles.clear();
    ...
    public void onChatMessage(ChatMessage event) {
        ...
        if (msg.equals("you can't light a fire here.")) {
            netoWoodcuttingScript.cannotLightFire = true;
        }
    ...
    public void onConfigChanged (ConfigChanged ev){
        if (ev.getGroup().equals(NetoWoodcuttingConfig.configGroup)) {
    ...
    public WoodcuttingTree getSelectedTree() {
        if (netoWoodcuttingScript != null) {
            return netoWoodcuttingScript.getActiveTree();
        }
        return config.TREE();
    }
