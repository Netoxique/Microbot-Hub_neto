...
public enum NetoFletchingItem
{
    ...
    public char getOption(NetoFletchingMaterial material, NetoFletchingMode fletchingMode) {
        if (fletchingMode == NetoFletchingMode.STRUNG
                || fletchingMode == NetoFletchingMode.PROGRESSIVE_STRUNG) {
            return '1';
        }
        if (material == NetoFletchingMaterial.LOG && option == '2') return '3';
        if (material == NetoFletchingMaterial.LOG && option == '3') return '4';
        //redwood is an exception
        if (material == NetoFletchingMaterial.REDWOOD)
            return '2';
        return option;
    }
}
