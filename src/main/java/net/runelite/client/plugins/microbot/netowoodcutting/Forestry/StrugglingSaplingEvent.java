package net.runelite.client.plugins.microbot.netowoodcutting.Forestry;
...
import net.runelite.client.plugins.microbot.netowoodcutting.NetoWoodcuttingPlugin;
import net.runelite.client.plugins.microbot.netowoodcutting.NetoWoodcuttingScript;
...
public class StrugglingSaplingEvent implements BlockingEvent {
    ...
    public StrugglingSaplingEvent(NetoWoodcuttingPlugin plugin) {
        this.plugin = plugin;
    }
        ...
        try{
            ...
            return strugglingSaplings.stream().anyMatch(obj ->
                    Rs2GameObject.hasAction(obj.getObjectComposition(), "Add-mulch") &&
                            obj.getWorldLocation().distanceTo(Rs2Player.getWorldLocation()) <= NetoWoodcuttingScript.FORESTRY_DISTANCE
            );
        } catch (Exception e) {
    ...
    public boolean execute() {
        try {
            ...
            while (this.validate()) {
                ...
                // If we don't know the correct ingredient, try the closest untried one
