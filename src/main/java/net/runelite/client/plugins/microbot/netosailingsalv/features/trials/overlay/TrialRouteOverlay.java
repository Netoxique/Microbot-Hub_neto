package net.runelite.client.plugins.microbot.netosailingsalv.features.trials.overlay;
...
import net.runelite.client.plugins.microbot.netosailingsalv.features.trials.BoatLocation;
import net.runelite.client.plugins.microbot.netosailingsalv.features.trials.TrialsScript;
import net.runelite.client.plugins.microbot.netosailingsalv.features.trials.data.PortalDirection;
import net.runelite.client.plugins.microbot.netosailingsalv.features.trials.data.TrialRoute;
...
public class TrialRouteOverlay extends Overlay {
    ...
    private final TrialsScript netoTrialsScript;

    @Inject
    public TrialRouteOverlay(Client client, NetoSailingSalvConfig config, TrialsScript netoTrialsScript) {
        super();
        setPosition(OverlayPosition.DYNAMIC);
        setLayer(OverlayLayer.ABOVE_SCENE);
        this.client = client;
        this.config = config;
        this.netoTrialsScript = netoTrialsScript;
    }
    ...
    public Dimension render(Graphics2D graphics) {
        ...
        var route = netoTrialsScript.getActiveTrialRoute();
        if (route == null) {
            return null;
        }
        ...
        var visiblePoints = netoTrialsScript.getVisibleActiveLineForPlayer(boatLocation, MAX_WAYPOINTS_TO_RENDER);
        if (visiblePoints == null || visiblePoints.isEmpty()) {
            return null;
        }

        var visibleIndices = netoTrialsScript.getNextUnvisitedIndicesForActiveRoute(MAX_WAYPOINTS_TO_RENDER);
        var portalDirection = netoTrialsScript.getVisiblePortalDirection(route);
    ...
    private void drawPortalLabel(Graphics2D graphics, Polygon polygon, PortalDirection portalDirection) {
        ...
        var label = portalDirection.Color.name() + " ↁE" + portalDirection.FirstMovementDirection.name();
