...
import net.runelite.client.plugins.microbot.netoagility.enums.AgilityCourse;
...
public class NetoAgilityScript extends Script
{

	final NetoAgilityPlugin plugin;
	NetoAgilityConfig config;
	...
	private final NetoAgilitySupplyManager supplyManager;
	private volatile boolean shuttingDown = false;

	@Inject
	public NetoAgilityScript(NetoAgilityPlugin plugin, NetoAgilityConfig config)
	{
		this.plugin = plugin;
		this.config = config;
		this.supplyManager = new NetoAgilitySupplyManager(plugin, config, this::isShuttingDown, this::shutdown);
	}
	...
	private void clearWalkingRouteForShutdown()
	{
		...
		{
			Rs2Walker.clearWalkingRoute("netoagility:shutdown");
			return;
		}

		Thread cleanupThread = new Thread(() -> Rs2Walker.clearWalkingRoute("netoagility:shutdown"), "NetoAgilityScript-shutdown-cleanup");
		cleanupThread.setDaemon(true);
		cleanupThread.start();
	}

	public boolean run(NetoAgilityConfig config)
	{
		this.config = config;
		return run();
	}
	...
	public boolean run()
	{
		...
		mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
			try
			{
				...
				NetoAgilitySupplyManager.InventorySnapshot inventorySnapshot = supplyManager.createInventorySnapshot();
