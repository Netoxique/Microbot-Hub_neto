...
@Slf4j
@PluginDescriptor(
        name = "Neto Mahogany Homes",
        description = "Automates Mahogany Homes contracts",
        tags = {"mahogany", "homes", "construction", "contract", "minigame", "s1d", "see1duck", "microbot"},
		authors = {"Neto"},
        version = NetoMahoganyHomesPlugin.version,
        minClientVersion = "2.0.0",
        cardUrl = "https://chsami.github.io/Microbot-Hub/NetoMahoganyHomesPlugin/assets/card.png",
        iconUrl = "https://chsami.github.io/Microbot-Hub/NetoMahoganyHomesPlugin/assets/icon.png",
...
public class NetoMahoganyHomesPlugin extends Plugin {
    ...
    @Getter
    @Inject
    private NetoMahoganyHomesConfig config;

    @Inject
    private OverlayManager overlayManager;

    @Inject
    private NetoMahoganyHomesOverlay textOverlay;

    @Inject
    private NetoPlankSackOverlay plankSackOverlay;

    @Inject
    private NetoMahoganyHomesHighlightOverlay highlightOverlay;

    @Inject
    private NetoMahoganyHomesScript script;
    ...
    @Provides
    NetoMahoganyHomesConfig provideConfig(ConfigManager configManager) {
        return configManager.getConfig(NetoMahoganyHomesConfig.class);
    }
    ...
    public void shutDown() {
        ...
        worldMapPointManager.removeIf(NetoMahoganyHomesWorldPoint.class::isInstance);
    ...
    public void onConfigChanged(ConfigChanged c) {
        if (!c.getGroup().equals(NetoMahoganyHomesConfig.GROUP_NAME)) {
            return;
        }

        switch (c.getKey()) {
            case NetoMahoganyHomesConfig.WORLD_MAP_KEY:
                worldMapPointManager.removeIf(NetoMahoganyHomesWorldPoint.class::isInstance);
                if (config.worldMapIcon() && currentHome != null) {
                    worldMapPointManager.add(new NetoMahoganyHomesWorldPoint(currentHome.getLocation(), this));
                }
                break;
            case NetoMahoganyHomesConfig.HINT_ARROW_KEY:
                client.clearHintArrow();
                clientThread.invoke(() -> {
    ...
    public void onOverlayMenuClicked(OverlayMenuClicked e) {
        ...
        if (e.getEntry().getOption().equals(NetoMahoganyHomesOverlay.CLEAR_OPTION)) {
            clientThread.invokeLater(() -> {
        ...
        if (e.getEntry().getOption().equals(NetoMahoganyHomesOverlay.RESET_SESSION_OPTION)) {
            sessionContracts = 0;
            sessionPoints = 0;
        }
    }
    ...
    private void applyCurrentHome(final Home h) {
        ...
        if (config.worldMapIcon()) {
            worldMapPointManager.removeIf(NetoMahoganyHomesWorldPoint.class::isInstance);
            worldMapPointManager.add(new NetoMahoganyHomesWorldPoint(h.getLocation(), this));
        }
    ...
    private void loadFromConfig() {
        final String group = NetoMahoganyHomesConfig.GROUP_NAME + "." + client.getAccountHash();
        final String name = configManager.getConfiguration(group, NetoMahoganyHomesConfig.HOME_KEY);
        if (name == null) {
            return;
        }
        ...
        try {
            ...
            applyCurrentHome(null);
            configManager.setConfiguration(group, NetoMahoganyHomesConfig.HOME_KEY, null);
        }
        ...
        final String tier = configManager.getConfiguration(group, NetoMahoganyHomesConfig.TIER_KEY);
        if (tier == null) {
            return;
        }
        ...
        try {
            ...
            contractTier = 0;
            configManager.unsetConfiguration(group, NetoMahoganyHomesConfig.TIER_KEY);
        }
    }
    ...
    private void updateConfig() {
        final String group = NetoMahoganyHomesConfig.GROUP_NAME + "." + client.getAccountHash();
        if (currentHome == null) {
            configManager.unsetConfiguration(group, NetoMahoganyHomesConfig.HOME_KEY);
            configManager.unsetConfiguration(group, NetoMahoganyHomesConfig.TIER_KEY);
        } else {
            configManager.setConfiguration(group, NetoMahoganyHomesConfig.HOME_KEY, currentHome.getName());
            configManager.setConfiguration(group, NetoMahoganyHomesConfig.TIER_KEY, contractTier);
        }
    }
